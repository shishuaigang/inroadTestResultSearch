...
class Prepare:
    ...
    def read_db(self):
        ...
        sql = "use Inroad_Test_Result select Response_time,TestNo from Inroad_Crawler_Test where API_URL = %s AND TestNo >= %s AND TestNo <= %s order by TestNo asc"
        res = []
    ...
    def detail(self):
        conn = pymssql.connect(host="192.168.31.99\\sql2012", user="sgshi", password="ssg12345!",
                               database="Inroad_Test_Result")
        cur = conn.cursor()
        cur.execute("use Inroad_Test_Result select distinct TestNo from Inroad_Crawler_Test order by TestNo asc")
        testNo = []
        APIname = []
        for row in cur:
            testNo.append(row[0])
        t1 = {"serial": testNo}
        conn.commit()
        cur.execute("use Inroad_Test_Result select distinct API_URL from Inroad_Crawler_Test")
        for row in cur:
